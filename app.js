...
async function v31Beep(freq=720,duration=.14){
 if(!v31Assist.sound)return false;
 try{
   if(!v31AudioCtx)v31AudioCtx=new (window.AudioContext||window.webkitAudioContext)();
   if(v31AudioCtx.state==='suspended')await v31AudioCtx.resume();
   const o=v31AudioCtx.createOscillator(),g=v31AudioCtx.createGain();
   o.frequency.value=freq;o.type='sine';
   const now=v31AudioCtx.currentTime;
   g.gain.setValueAtTime(.0001,now);
   g.gain.exponentialRampToValueAtTime(.12,now+.01);
   g.gain.exponentialRampToValueAtTime(.0001,now+duration);
   o.connect(g);g.connect(v31AudioCtx.destination);o.start(now);o.stop(now+duration+.02);
   return true;
 }catch(e){console.warn('Signalton nicht verfügbar',e);return false}
}
...
function openNavAssistSettings(){
 $('#modalBody').innerHTML=`
 <span class="tag">🔔 Navigationsassistenz · V3.6.0.1</span><h2>Hinweise unterwegs</h2>
 ...
 $('#v31TestAssist').onclick=()=>{v31Notify('turn');v32Speak('Test der Navigationsassistenz.',true)};
}
...
/* ===== V3.6.0.1: GPS-gesteuerte Live-Navigation ===== */
...
function v32Speak(text,force=false){
 ...
 try{
   ...
   u.lang='de-DE';u.rate=.95;u.pitch=1;u.volume=1;
   try{
   ...
   synth.speak(u);
   return true;
 }catch(e){console.warn('Sprachansage nicht verfügbar',e);return false}
}
...
function v32UpcomingTurns(limit=3){
 ...
 // V3.6.0.1: once a turn is completed, never select it again.
...
function openLiveNavCenter(){
 ...
 $('#modalBody').innerHTML=`
 <span class="tag">🧭 Live-Navigation · V3.6.0.1</span><h2>GPS-Navigation</h2>
 ...
 if($('#v32VoiceTest'))$('#v32VoiceTest').onclick=()=>{
   v32Speak('Test der Sprachansage. In 50 Metern rechts abbiegen.',true);
 };
}
...
/* ===== V3.6.0.1: Navigations-Simulator ===== */
let v321Sim={
 active:false,
 running:false,
 speedKmh:4,
...
function v321ApplySimPosition(){
 ...
 // V3.6.0.1: A simulator run is governed by the GPX end, not by an
 ...
 if(v321Sim.active && v321Sim.alongKm < __simTotal-0.015){
   ...
   // V3.6.0.1: every virtual GPS fix must drive the same navigation UI/update
...
function openSimulatorCenter(){
 ...
 $('#modalBody').innerHTML=`
 <span class="tag">🧪 Navigations-Simulator · V3.6.0.1</span><h2>Tour zuhause testen</h2>
 <div class="simCard">
...
/* ===== V3.6.0.1: Tour-Aufzeichnung & Live-Statistik ===== */
...
function v33FinishRecording(){
 try{
   ...
   console.error('V3.6.0.1 save error',e);
...
function v33TrackGpx(track){
 ...
 return `<?xml version="1.0" encoding="UTF-8"?><gpx version="1.1" creator="Trek & Sleep V3.6.0.1" xmlns="http://www.topografix.com/GPX/1/1"><trk><name>${name}</name><trkseg>${seg}</trkseg></trk></gpx>`;
}
...
function openTrackCenter(){
 ...
 $('#modalBody').innerHTML=`
 <span class="tag">⏺ Tour-Aufzeichnung · V3.6.0.1</span><h2>Gelaufenen Track aufzeichnen</h2>
...
/* ===== V3.6.0.1: Jagdzeiten & Sicherheitswarnungen ===== */
...
function v34InjectHud(){
 ...
 if(old){
   if(old.outerHTML!==html)old.outerHTML=html;
 }else host.insertAdjacentHTML('afterbegin',html);
}
function openHuntingCenter(){
 try{
   ...
   const speciesHtml=a.region?.supported
     ...
     : `<div class="huntNotice"><b>Noch keine Jagdzeiten für diese Region hinterlegt.</b><small>V3.6.0.1 startet mit Rheinland-Pfalz. Weitere Bundesländer können später ergänzt werden.</small></div>`;

   body.innerHTML=`
    <span class="tag">🦌 Jagd & Sicherheit · V3.6.0.1</span><h2>Jagdhinweise entlang der Tour</h2>
   ...
   const modal=$('#modal'),body=$('#modalBody');
   if(modal&&body){
     body.innerHTML=`<span class="tag">🦌 Jagd & Sicherheit · V3.6.0.1</span><h2>Jagdcenter</h2>
...
/* ===== V3.6.0.1 runtime-safe GPS/navigation helpers ===== */
...
function updateTurnInstruction(){
 // V3.6.0.1: Abbiegehinweise nur bei einer wirklich aktiven Live-Tour anzeigen.
...
function openMapPlanner(){
 $('#modalBody').innerHTML=`<span class="tag">📍 Kartenplanung · V3.6.0.1</span>
...
function startNavigationSession(){
 ...
 // V3.6.0.1 starts in Vor-Tour mode until GPS is close enough to the planned start.
...
/* ===== V3.6.0.1: Trail-Navigation, Etappenplan, Offline-Tourpaket ===== */
...
function openTrailGuide(){
 ...
 $('#modalBody').innerHTML=`
 <span class="tag">🧭 Trail-Navigation · V3.6.0.1</span><h2>Abbiegehinweise</h2>
...
function openDayStagePlanner(){
 try{
   ...
   for(let i=0;i<pts.length-1;i++){
     ...
     const pois=v30PoisForSegment(a,b),rawSt=elevationStatsBetween(a,b);
     const st=rawSt||{gain:0,loss:0,min:null,max:null};
     const stop=(mapPlan.points||[]).find(p=>p.type==='stop'&&Math.abs(Number(p.alongKm)-b)<.001);
     const pauseMin=stop?Number(stop.pauseMin||stop.pauseMinutes||15):0;
     stages.push({a,b,distance:b-a,pois,st,pauseMin,hasElevation:!!rawSt});
   }
   $('#modalBody').innerHTML=`
   <span class="tag">🗓 Etappenplan · V3.6.0.1</span><h2>Deine Tour in Abschnitten</h2>
   ...
   <div class="stageMetric"><b>${s.hasElevation?'+'+Math.round(s.st.gain||0)+' m':'—'}</b><small>${s.hasElevation?'Aufstieg':'Höhendaten fehlen'}</small></div><div class="stageMetric"><b>${s.pois.length}</b><small>POIs entlang Etappe</small></div>
   ...
   console.error('Etappenplan konnte nicht geöffnet werden',e);
   $('#modalBody').innerHTML='<span class="tag">🗓 Etappenplan · V3.6.0.1</span><h2>Etappen</h2><div class="card"><b>Etappen konnten nicht berechnet werden.</b><p>Bitte Start und Ziel unter „Punkte planen“ prüfen.</p></div>';
   $('#modal').classList.remove('hidden');
 }
}
...
function openOfflineTourCenter(){
 ...
 $('#modalBody').innerHTML=`
 <span class="tag">📥 Offline-Tour · V3.6.0.1</span><h2>Tour für unterwegs vorbereiten</h2>
...
function v30UpdateNavHintBar(){
 ...
 // V3.6.0.1: niemals im Vor-Tour-/Anreisemodus anzeigen.
...
async function ensureElevationData(force=false){
 ...
 try{
   ...
   console.warn('V3.6.0.1 Höhenprofil:',e);
   return false;
 }
}
...
async function openElevationProfile(){
 $('#modalBody').innerHTML=`<span class="tag">⛰ Höhenprofil · V3.6.0.1</span><h2>Höhenprofil</h2>
 ...
 $('#modalBody').innerHTML=`<span class="tag">⛰ Höhenprofil · V3.6.0.1</span><h2>Tourprofil</h2>
 <div class="elevHero">
...
function openTourOverview(){
 ...
 $('#modalBody').innerHTML=`<span class="tag">🧭 Tour-Check · V3.6.0.1</span><h2>Tour-Zusammenfassung</h2>
...
async function saveCurrentTour(){
 if(!route.length)return;
 $('#modalBody').innerHTML=`<div class="savingOverlay"><span class="tag">📥 Offline · V3.6.0.1</span><h2>Tour wird vorbereitet</h2><b>POIs werden automatisch gesichert …</b><span class="muted">Du musst „POIs laden“ vorher nicht mehr antippen.</span></div>`;
 $('#modal').classList.remove('hidden');
 ...
 $('#modalBody').innerHTML=`<span class="tag">✓ Offline gespeichert · V3.6.0.1</span>
 ...
 <div class="warning">Kartenkacheln bleiben weiterhin ausgenommen. V3.6.0.1 speichert Route, POIs, Rechtsdaten und App-Oberfläche offline.</div>`;
}

function openTourLibrary(){
 const tours=savedTours();
 $('#modalBody').innerHTML=`<span class="tag">↗ Touren · V3.6.0.1</span><h2>Meine Touren</h2>
...
function openOfflineManager(id){
 ...
 $('#modalBody').innerHTML=`<span class="tag">📥 Offline · V3.6.0.1</span>
 ...
 <div class="warning">V3.6.0.1 speichert POIs jetzt automatisch mit der Tour. Die eigentliche Kartenfläche benötigt für einen vollständigen Offline-Modus später eine Kartenquelle, die Offline-Pakete ausdrücklich erlaubt.</div>`;
...
function openNavigationSettings(){
 $('#modalBody').innerHTML=`
 <span class="tag">🧭 Navigation · V3.6.0.1</span>
 <h2>Tourführung</h2>
 ...
 <div class="warning">V3.6.0.1 bietet GPS-basierte Tourführung und Warnungen, aber noch keine sprachgeführte Abbiege-Navigation. Sie folgt weiterhin dem importierten GPX-Track.</div>`;
 $('#modal').classList.remove('hidden');
}

function openLegalOverview(){
 $('#modalBody').innerHTML=`
 <span class="tag">⚖ Rechts-Layer · V3.6.0.1</span>
 <h2>Pfälzerwald</h2>
...
/* ===== V3.6.0.1 Tour-Cockpit / Anreise / Backup / Werkzeuge ===== */
...
function openTourCockpit(){
 ...
 $('#modalBody').innerHTML=`
 <span class="tag">🎛 Tour-Cockpit · V3.6.0.1</span>
...
function openV28Tools(message=''){
 ...
 $('#modalBody').innerHTML=`
 <span class="tag">🧰 Werkzeuge · V3.6.0.1</span>
 <h2>App & Route</h2>
