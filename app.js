...
function openNavAssistSettings(){
 $('#modalBody').innerHTML=`
 <span class="tag">🔔 Navigationsassistenz · V3.3</span><h2>Hinweise unterwegs</h2>
...
/* ===== V3.3: GPS-gesteuerte Live-Navigation ===== */
...
function v32UpcomingTurns(limit=3){
 ...
 // V3.3: once a turn is completed, never select it again.
...
function openLiveNavCenter(){
 ...
 $('#modalBody').innerHTML=`
 <span class="tag">🧭 Live-Navigation · V3.3</span><h2>GPS-Navigation</h2>
...
function v321ApplySimPosition(){
 ...
 // V3.3: A simulator run is governed by the GPX end, not by an
 ...
 if(v321Sim.active && v321Sim.alongKm < __simTotal-0.015){
   ...
   // V3.3: every virtual GPS fix must drive the same navigation UI/update
...
function openSimulatorCenter(){
 ...
 $('#modalBody').innerHTML=`
 <span class="tag">🧪 Navigations-Simulator · V3.3</span><h2>Tour zuhause testen</h2>
 <div class="simCard">
...
/* ===== V3.3: Tour-Aufzeichnung & Live-Statistik ===== */
...
function v33SaveHistory(list){
 localStorage.setItem(V33_HISTORY_KEY,JSON.stringify((list||[]).slice(0,20)));
}
...
function v33FinishRecording(){
 ...
 v33Track.finishedAt=Date.now();
 if(v33Track.paused&&v33Track.pausedAt){
   v33Track.pausedMs+=Date.now()-v33Track.pausedAt;
   v33Track.pausedAt=null;v33Track.paused=false;
 }
 const summary={
   id:'track_'+Date.now(),
   date:new Date().toISOString(),
   name:(currentRoute?.name||'Aufgezeichnete Tour'),
   distanceKm:Number(v33Track.totalKm.toFixed(2)),
   durationMs:v33ElapsedMs(),
   avgKmh:Number(v33AvgKmh().toFixed(1)),
   maxOffRouteM:Math.round(v33Track.maxOffRouteM),
   points:v33Track.points.length,
   source:v33Track.source||'GPS',
   trackPoints:v33Track.points
 };
 const history=v33LoadHistory();
 history.unshift(summary);v33SaveHistory(history);
...
function v33TrackGpx(track){
 ...
 return `<?xml version="1.0" encoding="UTF-8"?><gpx version="1.1" creator="Trek & Sleep V3.3" xmlns="http://www.topografix.com/GPX/1/1"><trk><name>${name}</name><trkseg>${seg}</trkseg></trk></gpx>`;
}
...
function openTrackCenter(){
 $('#modalBody').innerHTML=`
 <span class="tag">⏺ Tour-Aufzeichnung · V3.3</span><h2>Gelaufenen Track aufzeichnen</h2>
 <div class="trackCard ${v33Track.recording?'recording':''}">
   <div class="trackHead">
     <div><b style="font-size:22px">${v33Track.recording?(v33Track.paused?'Aufzeichnung pausiert':'Aufzeichnung läuft'):'Bereit zur Aufzeichnung'}</b><small>${v33Track.recording?`${v33Track.source||'GPS'} wird protokolliert.`:'Funktioniert mit echtem GPS und mit dem Simulator.'}</small></div>
     <span class="trackBadge ${v33Track.recording?'rec':''}">${v33Track.recording?'● REC':'bereit'}</span>
   </div>
   <div class="trackGrid">
   ...
   <div class="trackActions">
     <button id="v33Start" class="primary">${v33Track.recording?(v33Track.paused?'▶ Fortsetzen':'● Läuft'):'⏺ Aufzeichnung starten'}</button>
     <button id="v33Pause" class="toolBtn">${v33Track.paused?'▶ Fortsetzen':'⏸ Pause'}</button>
     <button id="v33Finish" class="toolBtn">🏁 Beenden & speichern</button>
 ...
 $('#v33Pause').onclick=v33PauseRecording;
 $('#v33Finish').onclick=v33FinishRecording;
...
/* ===== V3.3 runtime-safe GPS/navigation helpers ===== */
...
function updateTurnInstruction(){
 // V3.3: Abbiegehinweise nur bei einer wirklich aktiven Live-Tour anzeigen.
...
function openMapPlanner(){
 $('#modalBody').innerHTML=`<span class="tag">📍 Kartenplanung · V3.3</span>
...
function startNavigationSession(){
 ...
 // V3.3 starts in Vor-Tour mode until GPS is close enough to the planned start.
...
/* ===== V3.3: Trail-Navigation, Etappenplan, Offline-Tourpaket ===== */
...
function openTrailGuide(){
 ...
 $('#modalBody').innerHTML=`
 <span class="tag">🧭 Trail-Navigation · V3.3</span><h2>Abbiegehinweise</h2>
...
function openDayStagePlanner(){
 ...
 $('#modalBody').innerHTML=`
 <span class="tag">🗓 Etappenplan · V3.3</span><h2>Deine Tour in Abschnitten</h2>
...
function openOfflineTourCenter(){
 ...
 $('#modalBody').innerHTML=`
 <span class="tag">📥 Offline-Tour · V3.3</span><h2>Tour für unterwegs vorbereiten</h2>
...
function v30UpdateNavHintBar(){
 ...
 // V3.3: niemals im Vor-Tour-/Anreisemodus anzeigen.
...
async function ensureElevationData(force=false){
 ...
 try{
   ...
   console.warn('V3.3 Höhenprofil:',e);
   return false;
 }
}
...
async function openElevationProfile(){
 $('#modalBody').innerHTML=`<span class="tag">⛰ Höhenprofil · V3.3</span><h2>Höhenprofil</h2>
 ...
 $('#modalBody').innerHTML=`<span class="tag">⛰ Höhenprofil · V3.3</span><h2>Tourprofil</h2>
 <div class="elevHero">
...
function openTourOverview(){
 ...
 $('#modalBody').innerHTML=`<span class="tag">🧭 Tour-Check · V3.3</span><h2>Tour-Zusammenfassung</h2>
...
async function saveCurrentTour(){
 if(!route.length)return;
 $('#modalBody').innerHTML=`<div class="savingOverlay"><span class="tag">📥 Offline · V3.3</span><h2>Tour wird vorbereitet</h2><b>POIs werden automatisch gesichert …</b><span class="muted">Du musst „POIs laden“ vorher nicht mehr antippen.</span></div>`;
 $('#modal').classList.remove('hidden');
 ...
 $('#modalBody').innerHTML=`<span class="tag">✓ Offline gespeichert · V3.3</span>
 ...
 <div class="warning">Kartenkacheln bleiben weiterhin ausgenommen. V3.3 speichert Route, POIs, Rechtsdaten und App-Oberfläche offline.</div>`;
}

function openTourLibrary(){
 const tours=savedTours();
 $('#modalBody').innerHTML=`<span class="tag">↗ Touren · V3.3</span><h2>Meine Touren</h2>
...
function openOfflineManager(id){
 ...
 $('#modalBody').innerHTML=`<span class="tag">📥 Offline · V3.3</span>
 ...
 <div class="warning">V3.3 speichert POIs jetzt automatisch mit der Tour. Die eigentliche Kartenfläche benötigt für einen vollständigen Offline-Modus später eine Kartenquelle, die Offline-Pakete ausdrücklich erlaubt.</div>`;
...
function openNavigationSettings(){
 $('#modalBody').innerHTML=`
 <span class="tag">🧭 Navigation · V3.3</span>
 <h2>Tourführung</h2>
 ...
 <div class="warning">V3.3 bietet GPS-basierte Tourführung und Warnungen, aber noch keine sprachgeführte Abbiege-Navigation. Sie folgt weiterhin dem importierten GPX-Track.</div>`;
 $('#modal').classList.remove('hidden');
}

function openLegalOverview(){
 $('#modalBody').innerHTML=`
 <span class="tag">⚖ Rechts-Layer · V3.3</span>
 <h2>Pfälzerwald</h2>
...
/* ===== V3.3 Tour-Cockpit / Anreise / Backup / Werkzeuge ===== */
...
function openTourCockpit(){
 ...
 $('#modalBody').innerHTML=`
 <span class="tag">🎛 Tour-Cockpit · V3.3</span>
...
function openV28Tools(message=''){
 ...
 $('#modalBody').innerHTML=`
 <span class="tag">🧰 Werkzeuge · V3.3</span>
 <h2>App & Route</h2>
